...
public class UserServiceImpl implements UserService {
	...
	public Integer addFriend(Integer uid, Integer fid) {
		if (UserMapper.queryFriend(uid, fid) == 0) {
			return UserMapper.insertFriend(uid, fid);
		}
		return -1;

	}

	public Integer removeFriend(Integer uid, Integer fid) {
		return UserMapper.deleteFriend(uid,fid);
	}

	public Integer queryFriendCountByUid(Integer uid) {
		return UserMapper.queryFriendCountByUid(uid);
	}

	public List<User> getFriendPageByUid(pageQueryVo pageQueryVo, Integer uid) {
		return UserMapper.selectFriendPageByUid(pageQueryVo,uid);
	}
